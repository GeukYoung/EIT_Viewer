function [Result,DataSet] = FxParam_AT_v1_app(DataSet)
% input
%   data : EIT data 
% output
...
if DataSet.EIT.Settings.Elec_config ~= 0
    switch DataSet.EIT.Settings.Elec_config
        case 0
...
for cnt = 1:length(peaks.insp)-1
    ...
    Result(cnt).TVi_au = DataSet.EIT.RVS(peaks.insp(cnt)) - DataSet.EIT.RVS(peaks.exp(cnt));
    Result(cnt).TVe_au = DataSet.EIT.RVS(peaks.insp(cnt)) - DataSet.EIT.RVS(peaks.exp(cnt+1));
    Result(cnt).MV = Result(cnt).TVi_au * Result(cnt).RR;
    Result(cnt).dFRC = Result(cnt).TVi_au - Result(cnt).TVe_au;
    Result(cnt).AU2mL = 1;
...
for cnt = 1:length(peaks.insp)-1
    ...
    Result(cnt).Im_TV = FxRecon_AT(-(-DataSet.EIT.ohm_HPF(:,peaks.exp(cnt)))*median([Result.AU2mL]));
    Result(cnt).TVi_au2ml_same = Result(cnt).TVi_au*median([Result.AU2mL]);
...
%% SQI
% dFRC
sqi.dFRC = abs([Result.TVi_au]-[Result.TVe_au])./(([Result.TVi_au]+[Result.TVe_au])/2);
